...
import { updateCommandePanier, envoieEmail, CommandeAnnuler } from "@/services/ClientService";
...
export default function CommandeEtape() {
        ...
        try {
            const emailData = {
                ...
                commande: {
                    ...
                    dateLivraison: formData.etape3.dateLivraison, // Ajout de la date de livraison
    ...
    useEffect(() => {
        if (user && user.emailUsers) {
            ...
            const dateL = localStorage.getItem("dateLivraison");
            
            if (methLivr && methPaiement && dateL && refCommande) {
                setFormData((prevData) => ({
                    ...prevData,
                    etape3: {
        ...
        try {
            ...
            if (panier.length > 0) {
                ...
                if (refCommandeNettoyee) {
                    ...
                    if (!formData.etape3.dateLivraison) {
                        setMessage({
                            ouvre: true,
                            texte: "Veuillez sélectionner une date de livraison",
                            statut: "error",
                        });
                        setOpen(true);
                        setLoading(false);
                    ...
                    const dataCommandeUpdate = {
                        ...
                        dateLivraison: formData.etape3.dateLivraison, 
                    ...
                    if (response.data) {
                        ...
                        try {
                            await envoyerEmailConfirmation(response.data, formData.etape1.email);
                        } catch (emailError) {
                        ...
                        setMessage({
                            ouvre: true,
                            texte: "Votre commande a été passée avec succès. En attente de votre paiement.",
                            statut: "success",
                        });
                        setOpen(true);
                        ...
                        localStorage.removeItem('fraisLivraison');
                        
                        setItems([]);
                        handleNext();
                    } else {
                        console.log("Erreur Backend: ", response.error);
                        setMessage({
                            ouvre: true,
                            texte: "Une erreur s'est produite lors de la validation de votre commande. Veuillez réessayer.",
                            statut: "error",
                        });
                        setOpen(true);
                    }
                    ...
                    setMessage({
                        ouvre: true,
                        texte: "Vous n'avez pas de commande à créer! Veuillez sélectionner vos produits.",
                        statut: "warning",
                    });
                    setOpen(true);
                }
                ...
                setMessage({
                    ouvre: true,
                    texte: "Votre panier est vide, veuillez sélectionner vos produits.",
                    statut: "warning",
                });
                setOpen(true);
            }
    ...
    const AnnulerCommande = async () => {
        ...
        try {
            ...
            if (refCommandeNettoyee) {
                console.log("refCommandeNettoyee: ", refCommandeNettoyee);
                const result = await CommandeAnnuler(refCommandeNettoyee);
                if (result.data) {
                    console.log("Annulation Success: ", result.data);
                    ...
                    console.log("Problème d'annulation: ", result.error);
                }
            }
            setAnnulation(false);
        } catch (error) {
            console.log("Problème de fonction AnnulerCommande:", error);
        } finally {
    ...
    function getStepContentComponent(step) {
        ...
        switch (step) {
            ...
            case 2:
                return <FormPaiement {...commonProps} />;
            case 3:
                return (
                    <FormValidationFinal
                        formData={formData}
                        ...
                        NetPayer={NetPayer}
                        loading={loading}
                        formatDateForDisplay={formatDateForDisplay} // Passer la fonction de formatage
                    />
                );
            default:
                                        ...
                                        {index === steps.length - 1 && (
                                            <div className="flex gap-6">
                                                <Button
                                                    ...
                                                    disabled={commandeExistante || loading}
                                                >
                                                    {loading ? (
                                                ...
                                                <Button
                                                    ...
                                                    disabled={commandeExistante || annulation}
                                                >
                                                    {annulation ? (
                                                        <div className="flex items-center gap-2">
                                                            <span className="loading loading-spinner"></span>
                    ...
                    {activeStep === steps.length && (
                        ...
                        >
                            <Typography className="text-lg font-semibold text-green-700 dark:text-green-300">
                                Toutes les étapes sont terminées - Votre commande est en cours de preparation !
                            </Typography>
                            <Link
                                to={"/Produit"}
                ...
                <Box className="sticky top-48 my-4 h-fit w-full rounded-2xl bg-transparent p-6 shadow-2xl dark:shadow-slate-950 lg:w-1/3">
                    ...
                    {items.length === 0 ? (
                        ...
                        <Box className="space-y-4">
                            ...
                            {(formData.etape3.methodeLivraison || formData.etape3.methodePaiement || formData.etape3.dateLivraison) && (
                                <div className="mt-4 rounded-lg bg-blue-50 p-3 dark:bg-blue-900/30">
                                    <div className="mb-2 flex justify-center font-bold text-blue-700 dark:text-blue-300">
                                    ...
                                    {formData.etape3.dateLivraison && (
                                        <div className="mt-2 text-sm text-blue-600 dark:text-blue-400">
                                             Date livraison: {formatDateForDisplay(formData.etape3.dateLivraison)}
                                        </div>
                                    )}
                                </div>
                            )}
...
//                     const result = await CommandeAnnuler(refCommandeNettoyee)
//                     if (result.data){
//                         console.log("Annulation Succees: ", result.data)
//                         localStorage.removeItem('panier');
//                         localStorage.removeItem('RefCommande');
//                         localStorage.removeItem('DataAdresse');
//                         localStorage.removeItem('methodeLivraison');
...
//                     }
//                     else{
//                         console.log("Probleme d'annualation: ", result.error)
//                     }
//                 }
//                 setAnnulation(false)
//         }catch(error){

//             console.log("PRobleme de fonction AnnulerCommande:", error)
//         }
//         finally{
//                         localStorage.removeItem('panier');
//                         localStorage.removeItem('RefCommande');
